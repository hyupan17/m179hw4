...
//void update(double **f, int width, int height);
void update_steady(double **f, int width, int height);
void heatflow(double **f, int width, int height, int num);
void update(double **f, int width, int height);
double error_inf(double **f, double **nf, int width, int height);
int reduce(int a, int length);
...
//prob2
  ...
  //heatflow(f, width, height, 8000);
  //cout << f[width/2][height/2];
  //update_steady(f, width, height);
  ...
  // for(j = 0; j < width; j++) {
  //   for(i = 0; i < height; i++) {
  //     if(i%4 == 0 || j%9==0 || (j+1)%9 == 0 || (j+2)%9 == 0 ) {
  //       f[j][i] = 0.5;
  //     } else if((i >= 10 && i <=50) && (j >= 10 && j <=30 ) ){
  //       f[j][i] = 0.5;
  //     }
  //   }
  // }

  //  double **temp=matrix(width, height);
  //  heatflow(f, width, height, temp, 40);

  //update_steady(f, width, height);
  // writing picture
  ofstream outfile("result2.bmp", ios::out | ios::binary);
...
double error_inf(double **f, double **nf, int width, int height) {
  ...
  for(j = 0; j < width; j++) {
    for(i= 0; i < height; i++) {
      if(fabs(f[j][i]-nf[j][i]) > result) result = fabs(f[j][i]-nf[j][i]);
    }
  }
  return result;
}
...
void update(double **f, int width, int height) {
  int i,j;
  double **nf = matrix(width, height);
  for(j = 0; j < width; j++) {
    for(i = 0; i < height; i++ ) {
      nf[j][i] = (f[j][reduce(i-1, height)] + f[j][reduce(i+1, height)] + f[reduce(j-1, width)][i] + f[reduce(j+1, width)][i])/4;
    }
  }
  for(i = 0; i < width; i++) {
    for(j = 0; j < height; j++) {
...
void update_steady(double **f, int width, int height) {
  ...
  double error = error_inf(f, nf, width, height);
  while(error > 0.001) {
    for(j = 0; j < width; j++) {
      for(i = 0; i < height; i++) {
        nf[j][i]=(f[j][reduce(i-1, height)]+f[j][reduce(i+1, height)] + f[reduce(j-1, width)][i] + f[reduce(j+1, width)][i])/4;
    ...
    error = error_inf(f, nf, width, height);
    for(j=0;j < width; j++) {
      for(i=0; i<height; i++) {
        f[j][i] = nf[j][i];
      }
...
// void update(double **f, int width, int height, int count) {
//   int i, j, time;
//   double **nf = matrix(width, height);
//   for(tim = )
// }
